  ...
  for (i = 0, section = filedata->section_headers;
	...
	printf ("%s\n      ", printable_section_name (filedata, section));
      else
      {
		...
		{
			//CHANGE FOR ATAM_HW3
			int section_name_size = strlen(SECTION_NAME_PRINT (section));			
			char * s_org = SECTION_NAME_PRINT (section);			
			char  *s_temp = malloc(section_name_size + 6 +1);
			strcpy(s_temp,s_org);
			strncat(s_temp,"_<3LEE",6);
			print_symbol(-(17), s_temp);
			free(s_temp);	
		}
		else
		{
...
process_symbol_table (Filedata * filedata)
{
	...
	{			
	  ...
	  for (si = 0; si < num_syms; si++)
	  {
	    print_dynamic_symbol (filedata, si, symtab, section,
				  strtab, strtab_size);
		if(strcmp(get_symbol_binding\
		 (filedata, ELF_ST_BIND((symtab + si)->st_info)),"GLOBAL") == 0)
		 {
			 num_globals_symbs++;
		 }
	 }		  
  ...
  free (filedata->mipsxlat);
  filedata->mipsxlat = NULL; 
  if(num_globals_symbs == 0)
  {
	  printf("\nThere are no GLOBAL symbols in this file.\n");
  }
  else if(num_globals_symbs == 1)
  {
